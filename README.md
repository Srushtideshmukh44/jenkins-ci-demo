...
Configure Jenkins locally to build a CI pipeline that:
...
Builds Docker image only on Quality Gate success

Sends email notifications on success/failure

Uses secure, configurable, non-hardcoded values
...
The implementation strictly focuses on Continuous Integration (CI) as per the problem statement.
